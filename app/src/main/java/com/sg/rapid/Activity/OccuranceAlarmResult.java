...
import com.sg.rapid.Adapters.ReportsAdapter;
...
import com.sg.rapid.ReportServices.ReportResponse;
...
public class OccuranceAlarmResult  extends AppCompatActivity implements BottomNavigationView.OnNavigationItemSelectedListener{
    ...
    private void showResults(final Context mContext, OccuranceRequest data){
        ...
        OccuranceService.getResults(data, new ResponseListner() {
            @Override
            public void onSucess(Object response, int sttuscode) {
                SpinnerManager.hideSpinner(mContext);
                Response<List<ReportResponse>> mRes = (Response<List<ReportResponse>>)response ;
                List<ReportResponse> mData = mRes.body();

                ReportResponse dataobj = mData.get(0);
                List<ReportData> mLsit = dataobj.getData();

                if(mLsit.size() >0){
                    mAdapter = new ReportsAdapter(mContext,mLsit);
                    mReportsData.setVisibility(View.VISIBLE);
                    mReportsData.setAdapter(mAdapter);
